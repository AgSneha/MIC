...
contract('MICToken', function(accounts) {
	var tokenInstance;
	...
	it('initialises the contract with the correct values', function() {
		return MICToken.deployed().then(function(instance) {
			...
			assert.equal(name, 'MIC Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'MIC', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard, 'MIC Token v1.0', 'has the correct standard');
		});
	})
	...
	it('allocates the initial supply upon deployment', function() {
		...
		}).then(function(adminBalane) {
			assert.equal(adminBalane.toNumber(), 1000000, 'it allocates the initial supply to the admin account');
		});
	});
	...
	it('transfers token ownership', function() {
		return MICToken.deployed().then(function(instance) {
			tokenInstance = instance;
			//test the require statement first by transferring something larger than the sender's balance
	...
	it('handles delegated transfer', function() {
		return MICToken.deployed().then(function(instance) {
			...
			//try transferring something larger than balance
			return tokenInstance.transferFrom(fromAccount, toAccount, 9999, { from: spendingAccount });
		}).then(assert.fail).catch(function(err) {
			assert(err.message.indexOf('revert') >= 0, 'cannot tansfer value larger than balance');
